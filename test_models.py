...
from top_cat import add_labels_for_image_to_post_d, get_config, get_labelling_funtion
...
def get_posts_including_labels_and_correctness(labelling_function, config):
    post_dicts = [
        {"media_file": i, "label_to_find": i.split("/")[1]}
    ...
    for post in post_dicts:
        ...
        post["correct_label"] = (
            (
                ...
                or post["label_to_find"]
                not in LABELS_TO_CARE_ABOUT  # want 'galaxy' which is not a cat or dog
            )
            # Also make sure we didn't get dogs when we were looking for cats
            and not (LABELS_TO_CARE_ABOUT - set([post["label_to_find"]])).intersection(
                post["labels"]
            )
        )
        if not post["correct_label"]:
            print(
                f'#WARNING: Wrong label for {post["media_file"]}, expected {post["label_to_find"]}.',
                file=sys.stderr,
            )
    return post_dicts

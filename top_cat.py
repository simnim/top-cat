...
def get_config(config_file_loc="~/.top_cat/config.toml"):
    ...
    assert not final_config["POST_TO_SLACK_TF"] or (
        final_config["POST_TO_SLACK_TF"]
        and final_config["SLACK_API_TOKEN"]
        and final_config["SLACK_API_TOKEN"] != "YOUR__SLACK__API_TOKEN_GOES_HERE"
    ), "If you want to post to slack then you need to add an api key to the config file!"
    assert not final_config["POST_TO_FB_TF"] or (
        final_config["POST_TO_FB_TF"]
        and final_config["FB_PAGE_ACCESS_TOKEN"]
        and final_config["FB_PAGE_ACCESS_TOKEN"]
        != "YOUR__FB__PAGE_ACCESS_TOKEN_GOES_HERE"
...
def fix_imgur_url(url):
    ...
    if "imgur.com" in url:
        ...
        if "." not in url.split("/")[-1]:
            ...
            possible_media_links = set(
                re.findall(r'content="(http.{0,50}%s\.[^"?]+)[?"]' % imgur_id, req.text)
            )
            assert possible_media_links, (
                "can't find the media links in imgur url " + url
            )
            suffix_mapto_link = dict(
                zip(
                    [link.split(".")[-1] for link in possible_media_links],
...
def fix_giphy_url(url):
    if "gfycat.com" in url:
        ...
        return re.sub(
            ".*gfycat.com/([^-]+)-.*", r"https://thumbs.gfycat.com/\1-mobile.mp4", url
        )
    return url
        ...
        if reddit_json.get("data") is not None:
            ...
            if config["VERBOSE"]:
                print(
                    "Attempt",
                    attempt,
                    "at reddit api call failed. Trying again...",
                    file=sys.stderr,
                )
        sleep(0.1)
    assert (
...
def extract_frames_from_im_or_video(media_file, config):
    ...
    if mime_t.split("/")[0] == "video" or mime_t == "image/gif":
        ...
        if seconds_in_video > config["MAX_IMS_PER_VIDEO"]:
            frames_to_grab = np.linspace(
                0, frames_in_video - 1, num=config["MAX_IMS_PER_VIDEO"], dtype=int
            )
        else:
...
def cast_to_pil_imgs(img_or_vid):
    if issubclass(type(img_or_vid), Image.Image):
        ...
        return [
            ...
            for frame in img_or_vid
        ]
    else:
        print(
            "# WARNING: wtf is the image?",
            img_or_vid,
            type(img_or_vid),
            type(img_or_vid[0]),
            file=sys.stderr,
...
def maybe_repost_to_slack(post, label, config):
    if config["POST_TO_SLACK_TF"]:
        label_map_to_channel = dict(
            zip(config["LABELS_TO_SEARCH_FOR"], config["SLACK_CHANNELS"])
        )
        slack_payload = {
...
def update_config_with_args(config, args):
    ...
    args_keys_to_conig_keys = dict(
        zip(args.keys(), [a.strip("-").replace("-", "_").upper() for a in args.keys()])
    )
    # If the user specified an argument (ex --db-file), then update the config
    config.update(
        [
            (args_keys_to_conig_keys[argk], args[argk])
            for argk in args.keys()
            if args[argk] is not None
        ]
    )
...
def maybe_repost_to_social_media(reddit_response_json, top_cat_config, db_conn):
    ...
    for label_to_search_for in top_cat_config["LABELS_TO_SEARCH_FOR"]:
        ...
        if label_to_search_for in top_post["labels"]:
            already_reposted = QUERIES.did_we_already_repost(
                db_conn, post_id=top_post["post_id"], label=label_to_search_for
            )
            if not already_reposted:
                ...
                QUERIES.record_the_repost(
                    db_conn, post_id=top_post["post_id"], label=label_to_search_for
                )
                db_conn.commit()
                print(
                    f'Got a new top {label_to_search_for}: {top_post["title"]} {top_post["url"]}'
                )
